...
type LogFileOpts map[flog.Level]*LogFile

type Hook struct {
	defaultLogger *lumberjack.Logger
	formatter     flog.Formatter
	minLevel      flog.Level
	loggerByLevel map[flog.Level]*lumberjack.Logger
}

func NewHook(defaultLogger *LogFile, minLevel flog.Level, formatter flog.Formatter, opts *LogFileOpts) (*Hook, error) {
	...
	hook := Hook{
		...
		minLevel:      minLevel,
		formatter:     formatter,
		loggerByLevel: make(map[flog.Level]*lumberjack.Logger),
	}
...
func (hook *Hook) Fire(entry *flog.Entry) error {
...
func (hook *Hook) Levels() []flog.Level {
	return flog.AllLevels[:hook.minLevel+1]
}
